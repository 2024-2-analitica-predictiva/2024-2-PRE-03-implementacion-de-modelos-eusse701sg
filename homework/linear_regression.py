...
class LinearRegression:
    ...
    def fit(self, X, y):
        ...
        for _ in tqdm(range(self.num_epochs)): # tqdm es la barra para medir el tiempo de ejecucion

            # Compute the predictions
            y_pred = np.matmul(X, self.coefs_) + self.intercept_  # Hace la multplicación matricial
